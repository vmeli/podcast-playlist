  ...
  if(document.getElementById('audio').paused) {
    ...
    arrayMolPostFormat4.forEach(function(item) {
        ...
        if(item.classList.value.includes("active-audio")) {
          item.querySelector('button').classList.value = "icon-pause";
        }
  })
    ...
    document.getElementById('audio').pause();
     ...
     arrayMolPostFormat4.forEach(function(item) {
        //parentTargetElement.classList.toggle('active-audio');
        if(item.classList.value.includes("active-audio")) {
          item.querySelector('button').classList.value = "icon-play";
        }
  })
  }
}
...
function eventPodcast(e) {
  ...
  arrayMolPostFormat4.forEach(function(item) {
    if(item == parentTargetElement) {
      //parentTargetElement.classList.toggle('active-audio');
      parentTargetElement.classList.add('active-audio');
      console.log("igual");
    }else {
      item.classList.remove('active-audio');
      item.querySelector('button').classList.value="icon-play";
  ...
  headline = parentTargetElement.querySelector('.Format4__title').textContent.trim();
  ...
  if(sourceAudio.getAttribute('src') != audioplayer) {
      ...
      audio.play();
      targetElement.classList.value = "icon-pause";
      document.querySelector("#play").classList.value = "icon-pause";

  }else {
      console.log(targetElement.className == 'icon-play', "acá doy click");
    ...
    if(audio.paused) {
      ...
      console.log("estoy pausado", e.target, document.getElementById('audio').paused);
      document.querySelector("#play").classList.value = "icon-play";
      targetElement.classList.value = "icon-play";
      audio.pause();
    }
